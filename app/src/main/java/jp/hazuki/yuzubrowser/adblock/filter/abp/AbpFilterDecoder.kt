...
class AbpFilterDecoder {
    ...
    fun decode(reader: BufferedReader, url: String?): UnifiedFilterSet {
        ...
        val scriptletFilter = mutableListOf<ElementFilter>()
            ...
            when {
                trimmedLine[0] == '!' -> trimmedLine.decodeComment(url, info)
                    ...
                    if (matcher.matches() && !trimmedLine.contains("##^responseheader")) { // exception necessary to process responseheader correctly
                        // Scriptoria change: re-enabled. The host this was vendored from does no
                        // cosmetic filtering and short-circuited here, which left elementList empty
                        // and silently produced blocklists with no hiding rules at all.
                        decodeElementFilter(
                            matcher.group(1),
                            matcher.group(2),
                            matcher.group(3),
        ...
        return UnifiedFilterSet(
            info, elementDisableFilter, elementFilter, scriptletFilter, filterLists,
        )
    }
    ...
    private fun decodeElementFilter(
        ...
        scriptletList: MutableList<ElementFilter>,
    ) {
        if (body == null) return

        // Scriptoria change: "+js(name, args...)" asks for a scriptlet rather than a selector to
        // hide. Upstream dropped these; they are now carried in the same per-domain structure with
        // the call text in place of the selector, and routed to their own list.
        val scriptlet = body.startsWith("+js(") && body.endsWith(")")
        if (body.startsWith("+js") && !scriptlet) return
        ...
        // The call text without the "+js(" wrapper; arguments are parsed where it is injected.
        val payload = if (scriptlet) body.substring(4, body.length - 1).trim() else body.trim()
        val target = if (scriptlet) scriptletList else elementFilterList
        ...
        domainList.forEach {
            ...
            target += if (domain.endsWith('*')) {
                domain = domain.substring(0, it.length - 1)
                TldRemovedElementFilter(domain, isHide, isNot, payload)
            } else {
                PlaneElementFilter(domain, isHide, isNot, payload)
            }
        }
    }
...
*/
            options.forEach {
                ...
                when {
                    type > 0x00ff_ffff -> {
                        elementFilter = true
                        // Scriptoria change: record which exception this is. Without the bit the
                        // filter ends up with contentType TYPE_ALL, which shares no bits with the
                        // element-hide request types, so it could never match and $elemhide and
                        // $generichide behaved identically to having no exception at all.
                        contentType = contentType or type
                    }
                    type > 0 -> {
        ...
        // Scriptoria change: the early return here was upstream's way of dropping $elemhide and
        // $generichide, which the host it was vendored from had no use for. Dropping them means a
        // site's cosmetic-filtering exception is ignored and its own layout gets hidden, so the
        // filter is now built and routed to elementFilterList by the branch at the end.
    ...
    /**
     * Drops one option from a filter line, leaving the pattern and the other options intact.
     *
     * Scriptoria change: this used to strip the option name as a substring, which mangled the
     * line rather than removing the option — "$empty" became "$empt", "$mp4" became "$mp" and
     * "$redirect=noopjs" became "$=noopjs". The unknown option that produced then made the whole
     * filter be discarded, so $empty and $mp4 rules silently never blocked anything.
     */
