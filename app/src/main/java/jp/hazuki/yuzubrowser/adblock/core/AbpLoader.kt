...
//                ErrorReport.printAndWriteLog(e)
    ...
    /** Scriptoria change: takes a prefix so scriptlet rules can reuse the same record format. */
    fun loadAllElementFilter(prefix: String = ABP_PREFIX_ELEMENT) = sequence {
        entityList.forEach {
            if (!it.enabled) return@forEach
